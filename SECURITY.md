...
To report a security issue, please use the GitHub Security Advisory ["Report a Vulnerability"](https://github.com/sahma19/podsalsa/security/advisories/new) tab.
...
curl -L -O https://github.com/sahma19/podsalsa/releases/download/$VERSION/multiple.intoto.jsonl
...
curl -L -O https://github.com/sahma19/podsalsa/releases/download/$VERSION/$ARTIFACT
...
slsa-verifier verify-artifact \
  ...
  --source-uri github.com/sahma19/podsalsa \
  --source-tag $VERSION \
  $ARTIFACT
```
...
#   e.g. ghcr.io/sahma19/podsalsa:v0.2.0@sha256:...
IMAGE="${IMAGE}@"$(crane digest "${IMAGE}")

# verify the image
slsa-verifier verify-image \
  --source-uri github.com/sahma19/podsalsa \
  --provenance-repository ghcr.io/sahma19/signatures \
...
```bash
# download policy.cue
curl -L -O https://raw.githubusercontent.com/sahma19/podsalsa/main/policy.cue

# verify the image with cosign
COSIGN_REPOSITORY=ghcr.io/sahma19/signatures cosign verify-attestation \
  --type slsaprovenance \
...
Verification for ghcr.io/sahma19/podsalsa:v0.6.2@sha256:d6eaa2ae653ee894e7c1bb0e27d50bc94cb65ac65ace5daa8818ff0a30879cbc --
...
GitHub Workflow Repository: sahma19/podsalsa
...
```bash
COSIGN_REPOSITORY=ghcr.io/sahma19/signatures cosign verify \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
...
```bash
Verification for ghcr.io/sahma19/podsalsa@sha256:d6eaa2ae653ee894e7c1bb0e27d50bc94cb65ac65ace5daa8818ff0a30879cbc --
The following checks were performed on each of these signatures:
  - The cosign claims were validated
  - Existence of the claims in the transparency log was verified offline
  - The code-signing certificate was verified using trusted certificate authority certificates
...
curl -L -O https://github.com/sahma19/podsalsa/releases/download/$VERSION/checksums.txt

# verify the checksum file
cosign verify-blob \
	--certificate https://github.com/sahma19/podsalsa/releases/download/$VERSION/checksums.txt.pem \
	--signature https://github.com/sahma19/podsalsa/releases/download/$VERSION/checksums.txt.sig \
	--certificate-identity-regexp '^https://github.com/sahma19/podsalsa/.github/workflows/release.yml@refs/tags/v[0-9]+.[0-9]+.[0-9]+(-rc.[0-9]+)?$' \
	--certificate-oidc-issuer https://token.actions.githubusercontent.com \
	checksums.txt
```
...
The SBOMs of the container is attestated with Cosign and uploaded to the `ghcr.io/sahma19/sbom` repository. The SBOMs can be verified with the `cosign` binary.
...
curl -L -O https://raw.githubusercontent.com/sahma19/podsalsa/main/policy-sbom.cue
...
The following checks were performed on each of these signatures:
  ...
  - Existence of the claims in the transparency log was verified offline
  - The code-signing certificate was verified using trusted certificate authority certificates
Certificate subject: https://github.com/sahma19/podsalsa/.github/workflows/release.yml@refs/tags/v0.6.2
Certificate issuer URL: https://token.actions.githubusercontent.com
GitHub Workflow Trigger: push
GitHub Workflow SHA: b0e7f0a52697533f31ed6d8c6c3fa813449bff88
GitHub Workflow Name: goreleaser
